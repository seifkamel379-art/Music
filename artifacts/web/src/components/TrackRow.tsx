...
const C = {
  card: "#121212", secondary: "#1F1F1F", foreground: "#FFFFFF",
  primary: "#1DB954", mutedFg: "#B3B3B3", border: "#2A2A2A",
  destructive: "#F15E6C",
};
...
export default function TrackRow({ track, index, isCurrent, isPlaying, isFavorite, onPlay, onFavorite, onPlaylist, onDownload, onRemove }: Props) {
  const [imgErr, setImgErr] = useState(false);
  const cover = COVERS[index % 3];
  ...
  return (
    <div
      ...
      style={{
        ...
        margin: "0 18px 8px",
        border: `1px solid ${isCurrent ? C.primary : C.border}`,
        borderRadius: 20, padding: 10,
        background: isCurrent ? C.secondary : C.card,
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, direction: "rtl" }}>
        <div style={{
          color: C.foreground, fontSize: 14, fontWeight: 700,
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
        }}>{track.title}</div>
        <div style={{
          color: C.mutedFg, fontSize: 12, fontWeight: 500, marginTop: 3,
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
        }}>{track.artist} · {track.duration}</div>
      </div>
      ...
      >
        <button onClick={onFavorite} title="مفضلة" style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "flex" }}>
          {isFavorite
        ...
        <button onClick={onPlaylist} title="أضف للمكتبة" style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "flex" }}>
        ...
        <button onClick={onDownload} title="تحميل" style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "flex" }}>
        ...
        {onRemove && (
          <button onClick={onRemove} title="حذف" style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "flex" }}>
