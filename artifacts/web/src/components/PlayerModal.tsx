...
const C = {
  espresso: "#000000", gold: "#1DB954", primary: "#1DB954",
  muted: "#181818", mutedFg: "#B3B3B3",
};
...
export default function PlayerModal({ onClose }: Props) {
  ...
  if (!currentTrack) {
    return (
      <div style={{ ...modalStyle, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
  ...
  return (
    <div style={modalStyle}>
      ...
      <div style={{ width: "100%", display: "flex", alignItems: "center", paddingInline: 16, paddingBottom: 24 }}>
        <button onClick={onClose} style={closeBtnStyle}>
          <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
        </button>
        <div style={{ flex: 1, textAlign: "center" }}>
          ...
          <div style={{
            ...
            maxWidth: 200, margin: "2px auto 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
            direction: "rtl",
      ...
      {/* Art */}
      <div style={{ width: artSize, height: artSize, borderRadius: 28, overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.6)", marginBottom: 36 }}>
        ...
        {status.isBuffering && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 28 }}>
            <div style={{ color: "#fff", fontSize: 16, fontWeight: 700, direction: "rtl" }}>تحميل...</div>
          </div>
        )}
      </div>
      ...
      <div style={{ width: "100%", paddingInline: 24, marginBottom: 28, direction: "rtl" }}>
        <div style={{ color: "#fff", fontSize: 26, fontWeight: 700, letterSpacing: -0.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {currentTrack.title}
        </div>
        <div style={{ color: "rgba(255,255,255,0.65)", fontSize: 16, fontWeight: 500, marginTop: 6, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {currentTrack.artist}
        </div>
      </div>
      ...
      <div style={{ width: "100%", paddingInline: 24, marginBottom: 36 }}>
        <div
          ref={barRef}
          style={{ height: 44, display: "flex", alignItems: "center", cursor: "pointer", position: "relative" }}
        ...
        >
          <div style={{ width: "100%", height: 5, borderRadius: 3, background: "rgba(255,244,223,0.15)", overflow: "visible", position: "relative" }}>
            <div style={{ height: 5, borderRadius: 3, background: C.gold, width: `${displayProgress * 100}%` }} />
          </div>
          {/* Thumb */}
          <div style={{
            position: "absolute", top: "50%", transform: `translateX(calc(${displayProgress * 100}% - 10px)) translateY(-50%)`,
            width: 20, height: 20, borderRadius: "50%", background: C.gold,
            boxShadow: "0 0 8px #1DB95499",
            left: 0, pointerEvents: "none",
          }} />
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10 }}>
          <span style={{ color: "rgba(255,255,255,0.55)", fontSize: 13 }}>{fmt(currentTime)}</span>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 32, marginBottom: 28 }}>
        <button
          onClick={playPrev}
          disabled={!hasPrev}
          style={{ width: 54, height: 54, display: "flex", alignItems: "center", justifyContent: "center", background: "none", border: "none", cursor: hasPrev ? "pointer" : "default", opacity: hasPrev ? 1 : 0.35 }}
        >
        ...
        <button
          ...
          style={{
            width: 72, height: 72, borderRadius: 36, background: C.gold, border: "none", cursor: "pointer",
            display: "flex", alignItems: "center", justifyContent: "center",
            boxShadow: "0 0 24px #1DB95466",
          }}
        >
          {status.playing ? (
            <svg width="36" height="36" viewBox="0 0 24 24" fill={C.espresso}><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
          ) : (
            <svg width="36" height="36" viewBox="0 0 24 24" fill={C.espresso} style={{ marginLeft: 3 }}><polygon points="5 3 19 12 5 21 5 3"/></svg>
          )}
        </button>
        ...
        <button
          onClick={playNext}
          disabled={!hasNext}
          style={{ width: 54, height: 54, display: "flex", alignItems: "center", justifyContent: "center", background: "none", border: "none", cursor: hasNext ? "pointer" : "default", opacity: hasNext ? 1 : 0.35 }}
        >
      ...
      {queue.length > 1 && (
        <div style={{ textAlign: "center" }}>
          <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 13, direction: "rtl" }}>
            {currentIndex + 1} / {queue.length} من القائمة
          </span>
        </div>
      )}
    </div>
