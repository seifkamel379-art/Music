...
/* Equalizer bar heights: 16 bars with varied patterns */
const BAR_COUNT = 16;
const BAR_DELAYS = [0, 0.3, 0.6, 0.1, 0.5, 0.2, 0.7, 0.4, 0.15, 0.55, 0.35, 0.65, 0.25, 0.45, 0.8, 0.1];
const BAR_DURATIONS = [1.1, 0.9, 1.3, 0.8, 1.0, 1.2, 0.95, 1.15, 0.85, 1.05, 1.25, 0.75, 1.0, 0.9, 1.1, 0.7];
...
export default function PlayerModal({ onClose, isFav, onFavorite, onDownload, isDownloading }: Props) {
  ...
  const bg = themeMode === "dark" ? "#0d0d0d" : colors.background;
        ...
        {Array.from({ length: BAR_COUNT }).map((_, i) => (
          <div
            ...
            style={{
              ...
              background: "linear-gradient(to top, rgba(29,185,84,0.22), rgba(29,185,84,0.04))",
              animationName: status.playing ? "eq-pulse" : "none",
              animationDuration: `${BAR_DURATIONS[i]}s`,
              animationDelay: `${BAR_DELAYS[i]}s`,
              animationTimingFunction: "ease-in-out",
              animationIterationCount: "infinite",
              animationDirection: "alternate",
              height: status.playing ? "35%" : "4%",
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 24px 0", direction: "rtl" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ color: status.playing ? "#1DB954" : colors.mutedForeground, fontSize: 15, marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", transition: "color 0.4s ease" }}>
              {currentTrack.artist}
            </div>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 24px 0" }}>
          ...
          <button onClick={pauseOrResume} style={{ width: 66, height: 66, borderRadius: "50%", background: "#1DB954", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "transform 0.1s, box-shadow 0.3s", boxShadow: status.playing ? "0 4px 24px rgba(29,185,84,0.5)" : "0 4px 16px rgba(0,0,0,0.3)" }}>
            {status.playing
